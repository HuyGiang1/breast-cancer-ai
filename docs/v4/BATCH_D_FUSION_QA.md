...
| **Combined Score Semantics** | Claimed "Độ tin cậy của kết luận tổng hợp" and "Nguy cơ tổng hợp". | Displayed ambiguous `combined_confidence`. | Labeled as "Experimental Combined Score" (`combined_malignant_score`), $0.50$ software decision midpoint disclaimer, no clinical risk band. | `test_fusion_frontend_contract.js` (PASS) |
