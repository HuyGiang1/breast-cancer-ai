...
| **29** | **Multimodal Combined Inputs** | 30 ML features + mammogram image | 30 ML inputs + image input | Form data assembled | `POST /api/v1/predict/multimodal/` active | Heuristic combination | **RESTORED & ENHANCED (Batch D)** | Rebuilt into dual-branch converging workstation with 30 WDBC inputs and mammogram dropzone | `frontend/pages/multimodal.html`, `frontend/js/pages/multimodal.js` |
| **30** | **Multimodal Demo Presets** | One-click loader for combined demo case | None | Omitted | Demo data available | Compatible | **RESTORED (Batch D)** | Added Benign and Malignant research demo presets loading both branches | `frontend/js/pages/multimodal.js` |
| **31** | **Multimodal Two-Branch Result Cards** | Separate ML branch card (features) & DL card (heatmap) | Single basic combined card | Omitted branch breakdown | Backend returns `ml_result` and `dl_result` | Compatible | **RESTORED & ENHANCED (Batch D)** | Rendered independent branch cards with ML top contributors and DL Grad-CAM interactive viewer | `frontend/js/pages/multimodal.js` |
| **32** | **Multimodal Weighted Synthesis** | 40% ML / 60% DL synthesis narrative | Text mention of weights | Rendered in card | Supported | Heuristic software demo only | **RESTORED & CORRECTED (Batch D)** | Fixed probability-space bug; transparent formula visualizer using raw probabilities only; prominent Branch Disagreement panel | `backend/app/api/endpoints.py`, `frontend/js/pages/multimodal.js` |
| **33** | **Multimodal Result Actions & AI** | AI advice block, report button, AI handoff | None | Omitted | Backend returns advice | Compatible | **RESTORED (Batch D)** | Dedicated AI Educational Guidance card, View Analysis Report link, and contextual AI Guide handoff | `frontend/js/pages/multimodal.js`, `frontend/js/pages/advisor.js` |
| **34** | **Patient Search** | Filter patient list by patient name or notes | Live search input `#patientSearchInput` | Debounced filter over `full_name`, `id`, `notes` | `GET /api/v1/patients/` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Rebuilt with live search, modality filters, and multiple sort modes in modern toolbar | `frontend/js/pages/patients.js` |
| **35** | **Patient Create** | Form for full name, DOB, gender, clinical notes | Modal form with validation | Submits `POST /api/v1/patients/` | `POST /api/v1/patients/` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Accessible modal dialog with comprehensive demographic fields and inline error handling | `frontend/js/pages/patients.js`, `frontend/js/components/workspace.js` |
| **36** | **Patient Edit** | In-place edit form (`beginPatientEdit`) | Edit button opens pre-filled modal | Submits `PUT /api/v1/patients/{id}` | `PUT /api/v1/patients/{id}` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Polished modal edit dialog available on registry cards and patient detail view | `frontend/js/pages/patients.js`, `frontend/js/pages/patient-detail.js` |
| **37** | **Patient Delete** | Delete button with basic prompt() | Accessible modal dialog with safety disclaimer | Calls `DELETE /api/v1/patients/{id}` | `DELETE /api/v1/patients/{id}` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Safety-explicit modal explaining that predictions remain preserved in system logs | `frontend/js/pages/patients.js`, `frontend/js/components/workspace.js` |
| **38** | **Patient-Linked Analysis Handoff** | Quick buttons launching prediction for selected patient | 3-modality quick launch bar | Passes `?patient_id=<id>` to ML, DL, and Fusion | Supported by backend | Fully compatible | **RESTORED & ENHANCED (Batch E)** | 3 prominent quick launch buttons on patient cards and detail page linking to canonical routes | `frontend/js/pages/patients.js`, `frontend/js/pages/patient-detail.js` |
| **39** | **Patient Timeline & History Drill-down** | Detailed timeline of predictions for single patient | Longitudinal timeline with modality badges | Calls `patientService.get(id)` and `history(id)` | `GET /api/v1/patients/{id}/` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Dedicated single-patient endpoint retrieval, clinical notes, and chronological timeline | `frontend/js/pages/patient-detail.js`, `backend/app/api/endpoints.py` |
| **40** | **Prediction History: Modality & Label Filters** | Filter history by type (ML/DL/Fusion) & diagnosis | Filter controls for Modality & Keyword | Filters rows client-side | Supported | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Differentiated "My Activity" (Personal) vs "Analysis Activity" (Doctor) with modality & search filters | `frontend/js/pages/history.js` |
| **41** | **Prediction History: Doctor Patient-Name Filter** | Patient selector dropdown `#historyPatientSelect` | Patient selector dropdown populated from registry | Filters history by patient ID | Supported | Fully compatible | **RESTORED (Batch E)** | Dedicated patient selector dropdown for Doctor accounts in Activity view | `frontend/js/pages/history.js` |
| **42** | **Prediction History: Rich Summary Cards** | Detailed cards with top features, thumbnails, patient name | Rich activity cards with modality badges & probability | Formatted cards with report & print triggers | Persisted in `predictions` table | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Modality icon badges, diagnosis pills, probability meters, patient attribution, and print triggers | `frontend/js/components/workspace.js`, `frontend/js/pages/history.js` |
| **43** | **Reports: Prediction Report Listing** | Listing of past predictions with direct report view | Dedicated Analysis Reports view with filters | Fetches persisted history | `GET /api/v1/predictions/history/` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Filter by modality and patient, report metadata cards with View Full Report and Print / Save PDF | `frontend/js/pages/reports.js` |
| **44** | **Reports: Printable Live HTML Report** | Printable view with charts and institutional headers | Browser-native Print / Save PDF action bar | Print-specific CSS media query | `GET /api/v1/predictions/{id}/report/` active | Fully compatible | **RESTORED & ENHANCED (Batch E)** | Added top action bar with window.print() and print CSS rules hiding controls during PDF export | `backend/app/api/endpoints.py` |
...
| **49** | **Profile: Single-Session Sign Out** | Terminate current session token | Button "Sign out (this device)" | Calls `authService.logout()` | `POST /api/v1/auth/logout/` active | Fully compatible | **PRESERVED** | Verified in Phase 3A.7/3B QA | `frontend/js/pages/profile.js:L87` |
| **50** | **Profile: Logout All Devices** | "Đăng xuất mọi thiết bị" button | Accessible confirmation modal triggering `logoutAll()` | `POST /api/v1/auth/logout-all/` | `POST /api/v1/auth/logout-all/` active | Fully compatible | **RESTORED (Batch E)** | Added `authService.logoutAll()` and confirmation modal in `profile.js` | `frontend/js/pages/profile.js`, `backend/app/api/endpoints.py` |
...
- **Batch D — Multimodal Fusion Parity**: **COMPLETED (PASS)**.
  ...
  - Prominent Branch Disagreement panel displayed BEFORE combined score when model outputs conflict.
  - Transparent mathematical formula visualizer with raw values and $0.50$ software decision midpoint disclaimer.
