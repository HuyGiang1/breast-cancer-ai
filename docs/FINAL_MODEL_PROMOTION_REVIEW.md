...
ML uses raw probability; no calibration transform is part of its final contract. DL Platt calibration was selected for validation reliability, but `calibration_selection.json` explicitly preserves raw-probability operating points. The research files do not persist a standalone fitted Platt object, coefficient/intercept pair, or checksum for runtime loading. The existing `calibration_profile.json` instead encodes a legacy Custom CNN empirical/Isotonic profile and must not be reused.
...
| EfficientNet-B0 full | Raw sigmoid malignant output | 0.515 | Auto-discovers/priority-selects legacy artifacts, applies profile-driven empirical/Isotonic postprocess, has legacy threshold/profile | Runtime mismatch; Platt display artifact unavailable |
...
Both selected model files are present locally and match their recorded SHA-256. ML metadata/config/threshold agree. DL metadata/config/threshold agree on architecture, full representation, image size, preprocessing policy, raw output, and threshold. The missing item is a versioned runtime-loadable DL Platt artifact.
...
| DL calibration | Legacy empirical/Isotonic profile; optional runtime fitting | Platt selected; fitted artifact missing | Do not reuse/refit; supply frozen Platt artifact or define raw-only display contract |
...
- Missing frozen DL Platt parameters prevent a faithful calibrated-probability runtime output.
...
- **DL: BLOCKED.** The raw classification threshold is understood, but the selected Platt calibration lacks a frozen runtime-loadable artifact and current runtime calibration is incompatible legacy behavior. A future integration may proceed only after the missing frozen calibration artifact/probability-display decision is resolved without refitting or threshold reselection.
